...
model_x.add(Convolution2D(nb_filters-10, kernel_size_2d[0]+2, kernel_size_2d[1]+2,
                        border_mode='valid',
...
model_x.add(Convolution2D(nb_filters+20, kernel_size_2d[0], kernel_size_2d[1]))
model_x.add(Activation('relu'))
...
model_y.add(Convolution2D(nb_filters-10, kernel_size_2d[0]+2, kernel_size_2d[1]+2,
                        border_mode='valid',
                        input_shape=input_shape_2d))
...
model_y.add(Convolution2D(nb_filters+20, kernel_size_2d[0], kernel_size_2d[1]))
model_y.add(Activation('relu'))
...
model_z.add(Convolution2D(nb_filters-10, kernel_size_2d[0]+2, kernel_size_2d[1]+2,
                        border_mode='valid',
                        input_shape=input_shape_2d))
...
model_z.add(Convolution2D(nb_filters+20, kernel_size_2d[0], kernel_size_2d[1]))
model_z.add(Activation('relu'))
...
model_3d.add(Convolution3D(nb_filters+10,kernel_size_3d[0], kernel_size_3d[1], kernel_size_3d[2],
						 border_mode='valid'))
model_3d.add(Activation('relu'))
print("Output shape of 3rd convolution (3d):", model_3d.output_shape)
model_3d.add(Convolution3D(nb_filters+20,kernel_size_3d[0], kernel_size_3d[1], kernel_size_3d[2],
						 border_mode='valid'))
model_3d.add(Activation('relu'))
print("Output shape of 4th convolution (3d):", model_3d.output_shape)
...
for i in range(int(len(brains)/4)):
	...
	for it in range(nb_rep):
		...
		v.balance(bal_test)
		v.split(1)
		X_val_x = v.getData(img_types, "2dx", inp_dim_2d)[0]
		y_val = X_val_x[1]
		X_val_x = X_val_x[0]
		X_val_y = v.getData(img_types, "2dy", inp_dim_2d)[0][0]
		X_val_z = v.getData(img_types, "2dz", inp_dim_2d)[0][0]
		X_val_3d = v.getData(img_types, "3d", inp_dim_3d)[0][0]
		...
		for j in range(nb_epoch):
			...
			if (j+1)%1 == 0:
				val_loss = final_model.evaluate([X_val_x,X_val_y, X_val_z, X_val_3d], y_val)
				score = evaluate(final_model,[X_val_x, X_val_y, X_val_z, X_val_3d],y_val)
				tpr_val = score[1]
				tnr_val = score[2]
