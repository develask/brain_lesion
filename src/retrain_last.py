...
step = 9
bal_train = int(sys.argv[3])
inp_dim_2d = 35
inp_dim_3d = 11
img_types = ["flair","FA","anatomica"]

# dnn1_batches
def getDataSoftMax(tr):
...
#galician dnn batches
def getDataSigmoide(tr):
	tr.createSlices(step=step)
	tr.balance(bal_train)
	tr.split(1)
	X_train_x = tr.getData(img_types, "2dx", inp_dim_2d)[0]
...
model_name = model_path.split("/")[-1].split(".")[0] + "_finetuned_bal" + sys.argv[3]

getData = None
if "sigmoid" in model_path:
	getData = getDataSigmoide
else:
	getData = getDataSoftMax

mo = load_model(model_path)

for layer in mo.layers[:-1]:
	layer.trainable = False

m = Model(model_name, mo, getData)
m.leaveOneOut(20, 128, 0.005, 0.005,brain_id)
